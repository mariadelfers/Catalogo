...
<?php
	...
	// evaluacion llegada de parametros y operacion de insercion
		...
		$sql = "Select c.IDCliente, c.Nombre_Cliente, p.Descripcion, p.Precio, o.Cantidad, cp.Fehcha_Entrega
From producto p INNER JOIN (orden o INNER JOIN (compra cp INNER JOIN cliente c ON cp.Cliente_IDCliente = c.IDCliente) ON o.Compra_IDCompra = cp.IDCompra) ON p.IDProducto = o.Producto_IDProducto ORDER BY Fehcha_Entrega";
