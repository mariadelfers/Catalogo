...
<?php
	...
	// evaluacion llegada de parametros y operacion de insercion
		...
		$sql = "Select c.IDCliente, c.Nombre_Cliente, p.Descripcion, p.Precio, o.Cantidad, cp.Fecha_Entrega
From producto p INNER JOIN (orden o INNER JOIN (compra cp INNER JOIN cliente c ON cp.Cliente_IDCliente = c.IDCliente) ON o.Compra_IDCompra = cp.IDCompra) ON p.IDProducto = o.Producto_IDProducto
where cp.Cliente_IDCliente ='" . $IDCliente . "'";
	...
	//	foreach ($result as $row) {
			...
			$Fecha_Entrega[$pos_row] = $row['Fecha_Entrega'];

			$pos_row = $pos_row + 1;
		}

		presentaPedidos($num_rows, $IDCliente, $Nombre_Cliente, $Descripcion, $Precio, $Cantidad, $Fecha_Entrega);
	}
